...
% FORMAT chan      = spm_mb_appearance('BiasFieldStruct',C,d,Mat,reg,fwhm,scl,T,samp)
...
%==========================================================================
% BiasFieldStruct()
function chan = BiasFieldStruct(C,d,Mat,reg,fwhm,scl,T,samp)
if nargin < 7, T    = {}; end
...
for n=1:numel(dat)    
    ...
    if do_updt_bf
        ...
        chan        = spm_mb_appearance('BiasFieldStruct',C,df,dat(n).Mat,reg,fwhm,scl);
        dat(n).bf.T = {chan(:).T};
...
% Bias field related
if do_updt_bf    
    chan       = BiasFieldStruct(C,df,Mn,reg,fwhm,[],datn.bf.T,samp);
...
if samp > 1
    ...
    if do_updt_bf
        ...
        chan = BiasFieldStruct(C,df,Mn,reg,fwhm,[],datn.bf.T);
...
% Get bias field
chan       = BiasFieldStruct(C,df,Mat,reg,fwhm,[],datn.bf.T,samp);
[bf,pr_bf] = BiasField(chan,d);
